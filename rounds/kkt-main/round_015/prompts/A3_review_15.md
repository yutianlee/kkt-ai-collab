...
Write a compact but deep Stage B referee report of at least 1700 words. Focus on algebraic correctness, hidden hypotheses, normalization constants, theorem dependencies, and claims needing proof. For each reviewed peer, identify one strongest contribution, one most dangerous gap, and one concrete verification task. Include a score table and research-strategy adjustment. Do not include private scratchwork; only output the final referee report in raw Markdown. Treat uncited external theorem claims as unverified unless the prompt supplied a source; assign missing source checks to A1/A2.
...
- Minimum length: at least 1700 words.
- Minimum sections/headings: at least 8. Schema labels ending in `:` count.
